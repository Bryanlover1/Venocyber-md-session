...
const {
    default: Bryan_Tech,    useMultiFileAuthState,
    delay,
...
router.get('/', async (req, res) => {
    ...
    let num = req.query.number;
     ...
     try {
            let Pair_Code_By_Luffy_Tech = Bryan_Tech({
                auth: {
                    creds: state.creds,
            ...
            Pair_Code_By_Bryan_Tech.ev.on("connection.update", async (s) => {
               ...
               let session = await Pair_Code_By_Bryan_Tech.sendMessage(Pair_Code_By_Bryan_Tech.user.id, { text: '' + b64data });
...
_Don't Forget To Give Star To My Repo_`
 await Pair_Code_By_Bryan_Tech.sendMessage(Pair_Code_By_Bryan_Tech.user.id,{text: NARUTO_MD_TEXT},{quoted:session})
    ...
    return await LUFFY_MD_PAIR_CODE()
});
